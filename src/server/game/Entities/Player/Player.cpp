...
KillRewarder::KillRewarder (Player* killer, Unit* victim, bool isBattleGround) :
        // 1. Initialize internal variables to default values.
        _killer(killer), _victim(victim), _isBattleGround(isBattleGround), _isPvP(false), _group(killer->GetGroup()), _groupRate(1.0f), _maxLevel(0), _maxNotGrayMember(NULL), _count(0), _sumLevel(0), _isFullXP(false), _xp(0)
{
...
void KillRewarder::_RewardPlayer (Player* player, bool isDungeon)
{
    ...
    {
        const float rate = _group ? _groupRate * float(player->getLevel()) / _sumLevel :          // Group rate depends on summary level.
...
bool Player::Create (uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId*/)
{
    ...
    // set starting level
    uint32 start_level = getClass() != CLASS_DEATH_KNIGHT ? sWorld->getIntConfig(CONFIG_START_PLAYER_LEVEL) : sWorld->getIntConfig(CONFIG_START_HEROIC_PLAYER_LEVEL);
...
void Player::HandleDrowning (uint32 time_diff)
{
    ...
    {
        if (m_zoneUpdateId == 5144)
            return;
        if (m_zoneUpdateId == 5145)
            return;
        if (m_zoneUpdateId == 4815)
            return;
        if (m_zoneUpdateId == 4816)
            return;
...
GameObject* Player::GetGameObjectIfCanInteractWith (uint64 guid, GameobjectTypes type) const
{
    if (GameObject *go = GetMap()->GetGameObject(guid))
    {
        if (go->GetGoType() == type)
        {
            ...
            sLog->outDebug(LOG_FILTER_MAPS, "IsGameObjectOfTypeInRange: GameObject '%s' [GUID: %u] is too far away from player %s [GUID: %u] to be used by him (distance=%f, maximal 10 is allowed)", go->GetGOInfo()->name, go->GetGUIDLow(), GetName(), GetGUIDLow(), go->GetDistance(this));
        }
    }
    return NULL;
}
...
{
    return p == this || (GetGroup() != NULL && GetGroup() == p->GetGroup() && GetGroup()->SameSubGroup(this, p));
}
...
bool Player::addSpell (uint32 spell_id, bool active, bool learning, bool dependent, bool disabled)
{
    ...
    else if (itr != m_spells.end())
    {
        ...
        if (itr->second->state != PLAYERSPELL_REMOVED && itr->second->active == active && itr->second->dependent == dependent && itr->second->disabled == disabled)
        {
...
bool Player::IsNeedCastPassiveSpellAtLearn (SpellEntry const* spellInfo) const
{
    ...
    bool need_cast = (!spellInfo->Stances || (form && (spellInfo->Stances & (1 << (form - 1)))) || (!form && (spellInfo->AttributesEx2 & SPELL_ATTR2_NOT_NEED_SHAPESHIFT)));
...
void Player::removeSpell (uint32 spell_id, bool disabled, bool learn_low_rank)
{
    ...
    else
    {
        ...
        for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
        {
            ...
            if ((_spell_idx->second->learnOnGetSkill == ABILITY_LEARNED_ON_GET_RACE_OR_CLASS_SKILL && pSkill->categoryId != SKILL_CATEGORY_CLASS) ||          // not unlearn class skills (spellbook/talent pages)
            // lockpicking/runeforging special case, not have ABILITY_LEARNED_ON_GET_RACE_OR_CLASS_SKILL
            ((pSkill->id == SKILL_LOCKPICKING || pSkill->id == SKILL_RUNEFORGING) && _spell_idx->second->max_value == 0))
            {
...
void Player::RemoveArenaSpellCooldowns (bool removeActivePetCooldowns)
{
    ...
    for (itr = m_spellCooldowns.begin(); itr != m_spellCooldowns.end(); itr = next)
    {
        ...
        if (entry && entry->RecoveryTime <= 10 * MINUTE * IN_MILLISECONDS && entry->CategoryRecoveryTime <= 10 * MINUTE * IN_MILLISECONDS)
        {
            // remove & notify
...
void Player::CleanupChannels ()
{
    ...
    sLog->outDebug(LOG_FILTER_PLAYER_LOADING, "Player: channels cleaned up!");
}
...
void Player::ModifySkillBonus (uint32 skillid, int32 val, bool talent)
{
    ...
    if (talent)          // permanent bonus stored in high part
        ...
        // temporary/item bonus stored in low part
...
bool Player::RewardHonor (Unit *uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_PVP_TOKEN_ENABLE) && pvptoken)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if ((MapType == 1 && !InBattleground() && !HasByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP)) || (MapType == 2 && !HasByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP)) || (MapType == 3 && !InBattleground()))
                return true;
...
void Player::CastItemCombatSpell (Unit *target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item *item, ItemPrototype const * proto)
{
    ...
    for (uint8 e_slot = 0; e_slot < MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                sLog->outError("Player::CastItemCombatSpell(GUID: %u, name: %s, enchant: %i): unknown spell %i is casted, ignoring...", GetGUIDLow(), GetName(), pEnchant->ID, pEnchant->spellid[s]);
                continue;
            }
...
void Player::SendLoot (uint64 guid, LootType loot_type)
{
    ...
    sLog->outDebug(LOG_FILTER_PLAYER_LOADING, "Player::SendLoot");
        ...
        if (pProto)
        {
                        ...
                        if (pBag != pItem)
                        {
                            if (ItemPrototype const* pBagProto = pBag->GetProto())
                            {
                                if (pBagProto->Class == pProto->Class && (!swap || pBag->GetSlot() != eslot))
                                    return (pBagProto->SubClass == ITEM_SUBCLASS_AMMO_POUCH) ? EQUIP_ERR_CAN_EQUIP_ONLY1_AMMOPOUCH : EQUIP_ERR_CAN_EQUIP_ONLY1_QUIVER;
                            }
                        }
                    }
                }
            ...
            if (type == INVTYPE_2HWEAPON)
            {
                ...
                if (!CanTitanGrip())
                {
                    ...
                    if (offItem && (!not_loading || CanUnequipItem(uint16(INVENTORY_SLOT_BAG_0) << 8 | EQUIPMENT_SLOT_OFFHAND, false) != EQUIP_ERR_OK || CanStoreItem(NULL_BAG, NULL_SLOT, off_dest, offItem, false) != EQUIP_ERR_OK))
...
uint8 Player::CanBankItem (uint8 bag, uint8 slot, ItemPosCountVec &dest, Item *pItem, bool swap, bool not_loading) const
{
    ...
    {
        sLog->outError("Possible hacking attempt: Player %s [guid: %u] tried to move token [guid: %u, entry: %u] out of the currency bag!", GetName(), GetGUIDLow(), pItem->GetGUIDLow(), pProto->ItemId);
...
uint8 Player::CanUseItem (ItemPrototype const *proto) const
{
...
Item* Player::_StoreItem (uint16 pos, Item *pItem, uint32 count, bool clone, bool update)
{
    ...
    if (!pItem2)
    {
        ...
        if (pItem->GetProto()->Bonding == BIND_WHEN_PICKED_UP || pItem->GetProto()->Bonding == BIND_QUEST_ITEM || (pItem->GetProto()->Bonding == BIND_WHEN_EQUIPED && IsBagPos(pos)))
    ...
    {
        if (pItem2->GetProto()->Bonding == BIND_WHEN_PICKED_UP || pItem2->GetProto()->Bonding == BIND_QUEST_ITEM || (pItem2->GetProto()->Bonding == BIND_WHEN_EQUIPED && IsBagPos(pos)))
...
void Player::UpdateItemDuration (uint32 time, bool realtimeonly)
{
    ...
    sLog->outDebug(LOG_FILTER_PLAYER_SKILLS, "Player::UpdateItemDuration(%u, %u)", time, realtimeonly);
...
void Player::ApplyEnchantment (Item *item, EnchantmentSlot slot, bool apply, bool apply_dur, bool ignore_condition)
{
    ...
    if ((slot == SOCK_ENCHANTMENT_SLOT || slot == SOCK_ENCHANTMENT_SLOT_2 || slot == SOCK_ENCHANTMENT_SLOT_3) && !item->GetProto()->Socket[slot - SOCK_ENCHANTMENT_SLOT].Color)
    {
...
void Player::ApplyItemReforge (Item *item, bool apply)
{
    ...
    sLog->outDebug(LOG_FILTER_PLAYER_ITEMS, "ApplyItemReforge : item %u, reforge id %u, stat value %d, old stat %u (x%.1f %d), new stat %u (x%.1f %d), apply %d", item->GetEntry(), reforge_id, statBaseValue, reforge->oldstat, reforge->oldstat_coef, statValue[0], reforge->newstat, reforge->newstat_coef, statValue[1], apply);
    ...
    for (uint8 i = 0; i < INVENTORY_SLOT_BAG_END; ++i)
    {
        if (m_items[i])
        {
            for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
            {
                ...
                // If we're dealing with a gem inside a prismatic socket we need to check the prismatic socket requirements
                // rather than the gem requirements itself. If the socket has no color it is a prismatic socket.
                if ((slot == SOCK_ENCHANTMENT_SLOT || slot == SOCK_ENCHANTMENT_SLOT_2 || slot == SOCK_ENCHANTMENT_SLOT_3) && !m_items[i]->GetProto()->Socket[slot - SOCK_ENCHANTMENT_SLOT].Color)
                {
...
{
    if (SatisfyQuestRace(pQuest, false) && SatisfyQuestSkillOrClass(pQuest, false) && SatisfyQuestExclusiveGroup(pQuest, false) && SatisfyQuestReputation(pQuest, false) && SatisfyQuestPreviousQuest(pQuest, false) && SatisfyQuestNextChain(pQuest, false) && SatisfyQuestPrevChain(pQuest, false) && SatisfyQuestDay(pQuest, false) && SatisfyQuestWeek(pQuest, false) && !sDisableMgr->IsDisabledFor(DISABLE_TYPE_QUEST, pQuest->GetQuestId(), this))
    {
...
{
    return SatisfyQuestStatus(pQuest, msg) && SatisfyQuestExclusiveGroup(pQuest, msg) && SatisfyQuestRace(pQuest, msg) && SatisfyQuestLevel(pQuest, msg) && SatisfyQuestSkillOrClass(pQuest, msg) && SatisfyQuestReputation(pQuest, msg) && SatisfyQuestPreviousQuest(pQuest, msg) && SatisfyQuestTimed(pQuest, msg) && SatisfyQuestNextChain(pQuest, msg) && SatisfyQuestPrevChain(pQuest, msg) && SatisfyQuestDay(pQuest, msg) && SatisfyQuestWeek(pQuest, msg) && !sDisableMgr->IsDisabledFor(DISABLE_TYPE_QUEST, pQuest->GetQuestId(), this) && SatisfyQuestConditions(pQuest, msg);
}
...
bool Player::CanCompleteQuest (uint32 quest_id)
{
    if (quest_id)
    {
        ...
        // auto complete quest
        if ((qInfo->IsAutoComplete() || qInfo->GetQuestFlags() & QUEST_FLAGS_AUTOCOMPLETE) && CanTakeQuest(qInfo, false))
            return true;
...
bool Player::CanRewardQuest (Quest const *pQuest, bool msg)
{
    ...
    if (!pQuest->IsDFQuest() && !pQuest->IsAutoComplete() && !(pQuest->GetQuestFlags() & QUEST_FLAGS_AUTOCOMPLETE) && GetQuestStatus(pQuest->GetQuestId()) != QUEST_STATUS_COMPLETE)
        return false;
    ...
    if (pQuest->HasFlag(QUEST_SPECIAL_FLAG_DELIVER))
    {
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
        {
            if (pQuest->ReqItemCount[i] != 0 && GetItemCount(pQuest->ReqItemId[i]) < pQuest->ReqItemCount[i])
            {
                if (msg)
...
void Player::SetHomebind (WorldLocation const& /*loc*/, uint32 /*area_id*/)
{
    ...
    // update sql homebind
    CharacterDatabase.PExecute("UPDATE character_homebind SET map = '%u', zone = '%u', position_x = '%f', position_y = '%f', position_z = '%f' WHERE guid = '%u'", m_homebindMapId, m_homebindAreaId, m_homebindX, m_homebindY, m_homebindZ, GetGUIDLow());
}
...
bool Player::_LoadFromDB (uint32 guid, SQLQueryHolder * holder, PreparedQueryResult & result)
{
    ...
    if (ObjectMgr::CheckPlayerName(m_name) != CHAR_NAME_SUCCESS || (GetSession()->GetSecurity() == SEC_PLAYER && sObjectMgr->IsReservedName(m_name)))
    {
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    else if (transGUID)
    {
        ...
        if (!Trinity::IsValidMapCoord(GetPositionX() + m_movementInfo.t_pos.m_positionX, GetPositionY() + m_movementInfo.t_pos.m_positionY, GetPositionZ() + m_movementInfo.t_pos.m_positionZ, GetOrientation() + m_movementInfo.t_pos.m_orientation) ||
        // transport size limited
        m_movementInfo.t_pos.m_positionX > 250 || m_movementInfo.t_pos.m_positionY > 250 || m_movementInfo.t_pos.m_positionZ > 250)
        {
            sLog->outError("Player (guidlow %d) have invalid transport coordinates (X: %f Y: %f Z: %f O: %f). Teleport to bind location.", guid, GetPositionX() + m_movementInfo.t_pos.m_positionX, GetPositionY() + m_movementInfo.t_pos.m_positionY, GetPositionZ() + m_movementInfo.t_pos.m_positionZ, GetOrientation() + m_movementInfo.t_pos.m_orientation);
        ...
        else
        {
            ...
            if (!m_transport)
            {
                sLog->outError("Player (guidlow %d) have problems with transport guid (%u). Teleport to bind location.", guid, transGUID);
    ...
    if (time_diff > 0)
    {
        ...
        float bubble = fields[23].GetUInt8() > 0 ? bubble1 * sWorld->getRate(RATE_REST_OFFLINE_IN_TAVERN_OR_CITY) : bubble0 * sWorld->getRate(RATE_REST_OFFLINE_IN_WILDERNESS);
...
{
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Loading auras for player %u", GetGUIDLow());
    ...
    if (result)
    {
        do
        {
            ...
            if (pQuest)
            {
                ...
                if (pQuest->HasSpecialFlag(QUEST_SPECIAL_FLAG_TIMED) && !GetQuestRewardStatus(quest_id))
                {
                    AddTimedQuest(quest_id);
...
InstancePlayerBind* Player::BindToInstance (InstanceSave *save, bool permanent, bool load)
{
    if (save)
    {
        ...
        bind.perm = permanent;
        if (!load)
            sLog->outDebug(LOG_FILTER_PLAYER_LOADING, "Player::BindToInstance: %s(%d) is now bound to map %d, instance %d, difficulty %d", GetName(), GetGUIDLow(), save->GetMapId(), save->GetInstanceId(), save->GetDifficulty());
...
bool Player::Satisfy (AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        if (ar->item)
        {
            if (!HasItemCount(ar->item, 1) && (!ar->item2 || !HasItemCount(ar->item2, 1)))
                missingItem = ar->item;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetArkCoreString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, ObjectMgr::GetItemPrototype(missingItem)->Name1);
                else if (LevelMin)
...
bool Player::_LoadHomeBind (PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        if (MapManager::IsValidMapCoord(m_homebindMapId, m_homebindX, m_homebindY, m_homebindZ) && !bindMapEntry->Instanceable() && GetSession()->Expansion() >= bindMapEntry->Expansion())
            ok = true;
        else
    ...
    if (!ok)
    {
        ...
        CharacterDatabase.PExecute("INSERT INTO character_homebind (guid, map, zone, position_x, position_y, position_z) VALUES ('%u', '%u', '%u', '%f', '%f', '%f')", GetGUIDLow(), m_homebindMapId, m_homebindAreaId, m_homebindX, m_homebindY, m_homebindZ);
    }

    sLog->outStaticDebug("Setting player home position - mapid: %u, areaid: %u, X: %f, Y: %f, Z: %f", m_homebindMapId, m_homebindAreaId, m_homebindX, m_homebindY, m_homebindZ);
...
void Player::SaveToDB ()
{
    ...
    {
        ss << GetMapId() << ", " << (uint32) GetInstanceId() << ", " << uint32(uint8(GetDungeonDifficulty()) | uint8(GetRaidDifficulty()) << 4) << ", " << finiteAlways(GetPositionX()) << ", " << finiteAlways(GetPositionY()) << ", " << finiteAlways(GetPositionZ()) << ", " << finiteAlways(GetOrientation()) << ", ";
    }
    else
    {
        ss << GetTeleportDest().GetMapId() << ", " << (uint32) 0 << ", " << uint32(uint8(GetDungeonDifficulty()) | uint8(GetRaidDifficulty()) << 4) << ", " << finiteAlways(GetTeleportDest().GetPositionX()) << ", " << finiteAlways(GetTeleportDest().GetPositionY()) << ", " << finiteAlways(GetTeleportDest().GetPositionZ()) << ", " << finiteAlways(GetTeleportDest().GetOrientation()) << ", ";
    }
...
void Player::_SaveActions (SQLTransaction& trans)
{
    for (ActionButtonList::iterator itr = m_actionButtons.begin(); itr != m_actionButtons.end();)
    {
        ...
        case ACTIONBUTTON_NEW:
            trans->PAppend("INSERT INTO character_action (guid, spec, button, action, type) VALUES ('%u', '%u', '%u', '%u', '%u')", GetGUIDLow(), m_activeSpec, (uint32) itr->first, (uint32) itr->second.GetAction(), (uint32) itr->second.GetType());
            itr->second.uState = ACTIONBUTTON_UNCHANGED;
            ++itr;
            break;
        case ACTIONBUTTON_CHANGED:
            trans->PAppend("UPDATE character_action SET action = '%u', type = '%u' WHERE guid = '%u' AND button = '%u' AND spec = '%u'", (uint32) itr->second.GetAction(), (uint32) itr->second.GetType(), GetGUIDLow(), (uint32) itr->first, m_activeSpec);
            itr->second.uState = ACTIONBUTTON_UNCHANGED;
            ++itr;
            break;
...
void Player::_SaveMail (SQLTransaction& trans)
{
    ...
    for (PlayerMails::iterator itr = m_mail.begin(); itr != m_mail.end(); ++itr)
    {
        ...
        {
            trans->PAppend("UPDATE mail SET has_items = '%u', expire_time = '" UI64FMTD "', deliver_time = '" UI64FMTD "', money = '%u', cod = '%u', checked = '%u' WHERE id = '%u'", m->HasItems() ? 1 : 0, (uint64) m->expire_time, (uint64) m->deliver_time, m->money, m->COD, m->checked, m->messageID);
...
void Player::_SaveDailyQuestStatus (SQLTransaction& trans)
{
    ...
    for (uint32 quest_daily_idx = 0; quest_daily_idx < PLAYER_MAX_DAILY_QUESTS; ++quest_daily_idx)
        if (GetUInt32Value(PLAYER_FIELD_DAILY_QUESTS_1 + quest_daily_idx))
            trans->PAppend("INSERT INTO character_queststatus_daily (guid, quest, time) VALUES ('%u', '%u', '" UI64FMTD "')", GetGUIDLow(), GetUInt32Value(PLAYER_FIELD_DAILY_QUESTS_1 + quest_daily_idx), uint64(m_lastDailyQuestTime));

    if (!m_DFQuests.empty())
    {
        for (DFQuestsDoneList::iterator itr = m_DFQuests.begin(); itr != m_DFQuests.end(); ++itr)
        {
            trans->PAppend("INSERT INTO character_queststatus_daily (guid, quest, time) VALUES ('%u', '%u', '" UI64FMTD "')", GetGUIDLow(), (*itr), uint64(m_lastDailyQuestTime));
        }
    }
}
...
void Player::_SaveSkills (SQLTransaction& trans)
{
    ...
    for (SkillStatusMap::iterator itr = mSkillStatus.begin(); itr != mSkillStatus.end();)
    {
        ...
        {
        case SKILL_NEW:
            trans->PAppend("INSERT INTO character_skills (guid, skill, value, max) VALUES ('%u', '%u', '%u', '%u')", GetGUIDLow(), itr->first, value, max);
            break;
        case SKILL_CHANGED:
            trans->PAppend("UPDATE character_skills SET value = '%u', max = '%u'WHERE guid = '%u' AND skill = '%u' ", value, max, GetGUIDLow(), itr->first);
            break;
        default:
            break;
        }
...
void Player::_SaveConquestPointsWeekCap ()
{
    ...
    {
        CharacterDatabase.PExecute("INSERT INTO character_cp_weekcap (guid, source, maxWeekRating, weekCap) VALUES ('%u', '%u', '%u', '%u')", GetGUIDLow(), source, m_maxWeekRating[source], m_conquestPointsWeekCap[source]);
    }
}
...
void Player::_SaveStats (SQLTransaction& trans)
{
    ...
    ss << "INSERT INTO character_stats (guid, maxhealth, maxpower1, maxpower2, maxpower3, maxpower4, maxpower5, maxpower6, maxpower7, maxpower8, maxpower9, maxpower10, "
            ...
            "blockPct, dodgePct, parryPct, critPct, rangedCritPct, spellCritPct, attackPower, rangedAttackPower, spellPower) VALUES (" << GetGUIDLow() << ", " << GetMaxHealth() << ", ";
    for (uint8 i = 0; i < MAX_POWERS; ++i)
...
{
    std::ostringstream ss;
    ss << "UPDATE characters SET position_x='" << x << "', position_y='" << y << "', position_z='" << z << "', orientation='" << o << "', map='" << mapid << "', zone='" << zone << "', trans_x='0', trans_y='0', trans_z='0', " << "transguid='0', taxi_path='' WHERE guid='" << GUID_LOPART(guid) << "'";
...
void Player::RemovePet (Pet* pet, PetSlot mode, bool returnreagent)
{
    ...
    if (pet)
    {
        sLog->outDebug(LOG_FILTER_UNITS, "RemovePet %u, %u, %u", pet->GetEntry(), mode, returnreagent);
...
bool Player::ActivateTaxiPathTo (std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
    ...
    {
        if (node->map_id != GetMapId() || (node->x - GetPositionX()) * (node->x - GetPositionX()) + (node->y - GetPositionY()) * (node->y - GetPositionY()) + (node->z - GetPositionZ()) * (node->z - GetPositionZ()) > (2 * INTERACTION_DISTANCE) * (2 * INTERACTION_DISTANCE) * (2 * INTERACTION_DISTANCE))
        {
            WorldPacket data(SMSG_ACTIVATETAXIREPLY, 4);
...
void Player::ContinueTaxiFlight ()
{
    ...
    float distNext = (nodeList[0].x - GetPositionX()) * (nodeList[0].x - GetPositionX()) + (nodeList[0].y - GetPositionY()) * (nodeList[0].y - GetPositionY()) + (nodeList[0].z - GetPositionZ()) * (nodeList[0].z - GetPositionZ());
    ...
    for (uint32 i = 1; i < nodeList.size(); ++i)
    {
        ...
        distNext = (node.x - GetPositionX()) * (node.x - GetPositionX()) + (node.y - GetPositionY()) * (node.y - GetPositionY()) + (node.z - GetPositionZ()) * (node.z - GetPositionZ());

        float distNodes = (node.x - prevNode.x) * (node.x - prevNode.x) + (node.y - prevNode.y) * (node.y - prevNode.y) + (node.z - prevNode.z) * (node.z - prevNode.z);
...
inline bool Player::_StoreOrEquipNewItem (uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot, int32 price, ItemPrototype const *pProto, Creature *pVendor, VendorItem const* crItem, bool bStore)
{
    ...
    uint16 uiDest;
    uint8 msg = bStore ? CanStoreNewItem(bag, slot, vDest, item, pProto->BuyCount * count) : CanEquipNewItem(slot, uiDest, item, false);
    if (msg != EQUIP_ERR_OK)
    {
        SendEquipError(msg, NULL, NULL, item);
        return false;
...
void Player::SendInitialPacketsAfterAddToMap ()
{
    ...
    { SPELL_AURA_MOD_FEAR, SPELL_AURA_TRANSFORM, SPELL_AURA_WATER_WALK, SPELL_AURA_FEATHER_FALL, SPELL_AURA_HOVER, SPELL_AURA_SAFE_FALL, SPELL_AURA_FLY, SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED, SPELL_AURA_NONE };
...
bool Player::isHonorOrXPTarget (Unit* pVictim)
{
    ...
    {
        if (pVictim->ToCreature()->isTotem() || pVictim->ToCreature()->isPet() || pVictim->ToCreature()->GetCreatureInfo()->flags_extra & CREATURE_FLAG_EXTRA_NO_XP_AT_KILL)
            return false;
    }
    return true;
}
...
void Player::UpdateCorpseReclaimDelay ()
{
    ...
    if ((pvp && !sWorld->getBoolConfig(CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVP)) || (!pvp && !sWorld->getBoolConfig(CONFIG_DEATH_CORPSE_RECLAIM_DELAY_PVE)))
        return;
...
uint8 Player::CanEquipUniqueItem (Item* pItem, uint8 eslot, uint32 limit_count) const
{
    ...
    for (uint32 enchant_slot = SOCK_ENCHANTMENT_SLOT; enchant_slot < SOCK_ENCHANTMENT_SLOT + 3; ++enchant_slot)
    {
        ...
        uint32 gem_limit_count = !pItem->IsEquipped() && pGem->ItemLimitCategory ? pItem->GetGemCountWithLimitCategory(pGem->ItemLimitCategory) : 1;
...
void Player::HandleFall (MovementInfo const& movementInfo)
{
    ...
    if (z_diff >= 14.57f && !isDead() && !isGameMaster() && !HasAuraType(SPELL_AURA_HOVER) && !HasAuraType(SPELL_AURA_FEATHER_FALL) && !HasAuraType(SPELL_AURA_FLY) && !IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
    {
...
void Player::_SaveGlyphs (SQLTransaction& trans)
{
    ...
    {
        trans->PAppend("INSERT INTO character_glyphs VALUES('%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')", GetGUIDLow(), spec, m_Glyphs[spec][0], m_Glyphs[spec][1], m_Glyphs[spec][2], m_Glyphs[spec][3], m_Glyphs[spec][4], m_Glyphs[spec][5], m_Glyphs[spec][6], m_Glyphs[spec][7], m_Glyphs[spec][8]);
    }
}
...
void Player::UpdateSpecCount (uint8 count)
{
    ...
    if (count > curCount)
    {
        ...
        for (ActionButtonList::iterator itr = m_actionButtons.begin(); itr != m_actionButtons.end(); ++itr)
            trans->PAppend("INSERT INTO character_action (guid, button, action, type, spec) VALUES ('%u', '%u', '%u', '%u', '%u')", GetGUIDLow(), uint32(itr->first), uint32(itr->second.GetAction()), uint32(itr->second.GetType()), 1);
    }
