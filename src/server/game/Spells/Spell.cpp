...
void Spell::prepareDataForTriggerSystem (AuraEffect const * /*triggeredByAura*/)
{
    ...
    case SPELL_DAMAGE_CLASS_RANGED:
        ...
        break;
    default:
        if (m_spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON && m_spellInfo->EquippedItemSubClassMask & (1 << ITEM_SUBCLASS_WEAPON_WAND) && m_spellInfo->AttributesEx2 & SPELL_ATTR2_AUTOREPEAT_FLAG)          // Wands auto attack
        {
            m_procAttacker = PROC_FLAG_DONE_RANGED_AUTO_ATTACK;
            m_procVictim = PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK;
        }
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && (m_spellInfo->SpellFamilyFlags[0] & 0x18 ||          // Freezing and Frost Trap, Freezing Arrow
    ...
    {
        if (m_IsTriggeredSpell && (m_spellInfo->AttributesEx2 & SPELL_ATTR2_TRIGGERED_CAN_TRIGGER_PROC || m_spellInfo->AttributesEx3 & SPELL_ATTR3_TRIGGERED_CAN_TRIGGER_PROC_2))
...
struct ChainHealingOrder: public std::binary_function<const Unit*, const Unit*, bool>
{
    ...
    int32 ChainHealingHash (Unit const* Target) const
    {
        /*if (Target == MainTarget)
         return 0;
         else*/if (Target->GetTypeId() == TYPEID_PLAYER && MainTarget->GetTypeId() == TYPEID_PLAYER && Target->ToPlayer()->IsInSameRaidWith(MainTarget->ToPlayer()))
        {
...
void Spell::SearchChainTarget (std::list<Unit*> &TagUnitMap, float max_range, uint32 num, SpellTargets TargetType)
{
    ...
    while (num)
    {
        ...
        else
        {
            ...
            while ((m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE && !m_caster->isInFrontInMap(*next, max_range)) || !m_caster->canSeeOrDetect(*next) || !cur->IsWithinLOSInMap(*next) || (*next)->GetCreatureType() == CREATURE_TYPE_CRITTER || ((GetSpellInfo()->AttributesEx6 & SPELL_ATTR6_IGNORE_CROWD_CONTROL_TARGETS) && !(*next)->CanFreeMove()))
            {
                ++next;
...
void Spell::SelectEffectTargets (uint32 i, uint32 cur)
{
    ...
    default:
    {
        ...
        case TARGET_UNIT_DRIVER:
            ...
            break;
        default:
            sLog->outError("SPELL (caster[type: %u; guidlow: %u], spell: %u): unhandled spell target (%u)", m_caster->GetTypeId(), m_caster->GetGUIDLow(), m_spellInfo->Id, cur);
            break;
        }
        break;
    }
    ...
    else if (pushType)
    {
        ...
        default:
            ...
            case TARGET_UNIT_CLASS_TARGET:
            {
                Player* targetPlayer = m_targets.getUnitTarget() && m_targets.getUnitTarget()->GetTypeId() == TYPEID_PLAYER ? (Player*) m_targets.getUnitTarget() : NULL;
...
//                    }
            ...
            else
            {
                ...
                case 2812:          // Paladin Holy Wrath
                    if (m_spellInfo->Effect[i] == SPELL_EFFECT_APPLY_AURA && m_spellInfo->EffectApplyAuraName[i] == SPELL_AURA_MOD_STUN)
                    {
                        ...
                        for (std::list<Unit*>::iterator itr = unitList.begin(); itr != unitList.end();)
                        {
                            ...
                            if ((CREATURE_TYPE_DEMON == typeCreature || CREATURE_TYPE_UNDEAD == typeCreature) || (isElemsAndDragonkings && (CREATURE_TYPE_DRAGONKIN == typeCreature || CREATURE_TYPE_ELEMENTAL == typeCreature)))
                            {
                                itr++;
                            }
                            else
                ...
                if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && m_spellInfo->SpellFamilyFlags[0] & 0x00080000)
                {
                    ...
                    for (std::list<Unit*>::iterator itr = unitList.begin(); itr != unitList.end(); ++itr)
                    {
                        if ((*itr)->GetTypeId() == TYPEID_UNIT && (*itr)->GetOwnerGUID() == m_caster->GetGUID() && (*itr)->ToCreature()->GetCreatureInfo()->type == CREATURE_TYPE_UNDEAD)
                        {
                            unit_to_add = (*itr);
                            break;
                        }
...
void Spell::finish (bool ok)
{
    ...
    if (IsChanneledSpell(m_spellInfo) && m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        if (Unit *charm = m_caster->GetCharm())
            if (charm->GetTypeId() == TYPEID_UNIT && charm->ToCreature()->HasUnitTypeMask(UNIT_MASK_PUPPET) && charm->GetUInt32Value(UNIT_CREATED_BY_SPELL) == m_spellInfo->Id)
...
void Spell::TakePower ()
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        if (m_spellInfo->powerType == POWER_RAGE || m_spellInfo->powerType == POWER_ENERGY || m_spellInfo->powerType == POWER_RUNE || m_spellInfo->powerType == POWER_RUNIC_POWER)
...
bool ActsOfSacrificeCheck (Unit* caster)
{
    ...
    if (caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_PALADIN, 3022, 0) && caster->HasAuraType(SPELL_AURA_MOD_ROOT) || caster->HasAuraType(SPELL_AURA_MOD_DECREASE_SPEED))
        return true;
...
SpellCastResult Spell::CheckCast (bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR0_PASSIVE))
    {
        ...
        if (m_caster->ToPlayer()->HasSpellCooldown(m_spellInfo->Id) || (strict && !m_IsTriggeredSpell && m_caster->ToPlayer()->HasGlobalCooldown(m_spellInfo)))
        {
            if (m_triggeredByAuraSpell)
    ...
    {
        if (m_spellInfo->Attributes & SPELL_ATTR0_OUTDOORS_ONLY && !m_caster->GetMap()->IsOutdoors(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ()))
            return SPELL_FAILED_ONLY_OUTDOORS;

        if (m_spellInfo->Attributes & SPELL_ATTR0_INDOORS_ONLY && m_caster->GetMap()->IsOutdoors(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ()))
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->ToPlayer()->isMoving())
    {
        ...
        if ((!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) || m_spellInfo->Effect[0] != SPELL_EFFECT_STUCK) && (IsAutoRepeat() || (m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED) != 0) && !m_caster->CanCastWhileWalking(m_spellInfo))
    ...
    if (target)
    {
        ...
        else if (m_caster == target)
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)          // Target - is player caster
            {
                ...
                if (m_targets.getTargetMask() == TARGET_FLAG_SELF && m_spellInfo->EffectImplicitTargetA[1] == TARGET_UNIT_TARGET_ENEMY)
                {
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (m_spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN && (633 == m_spellInfo->Id || 642 == m_spellInfo->Id || 1022 == m_spellInfo->Id))
            {
    ...
    if ((m_spellInfo->AttributesEx4 & SPELL_ATTR4_NOT_USABLE_IN_ARENA) || (GetSpellRecoveryTime(m_spellInfo) > 10 * MINUTE * IN_MILLISECONDS && !(m_spellInfo->AttributesEx4 & SPELL_ATTR4_USABLE_IN_ARENA)))
    ...
    if (m_caster->GetTypeId() == TYPEID_UNIT || !m_caster->ToPlayer()->isGameMaster())
    {
        ...
        SpellCastResult locRes = sSpellMgr->GetSpellAllowedInLocationError(m_spellInfo, m_caster->GetMapId(), zone, area, m_caster->GetTypeId() == TYPEID_PLAYER ? m_caster->ToPlayer() : NULL);
        if (locRes != SPELL_CAST_OK)
            return locRes;
    }

    // not let players cast spells at mount (and let do it to creatures)
    if (m_caster->IsMounted() && m_caster->GetTypeId() == TYPEID_PLAYER && !m_IsTriggeredSpell && !IsPassiveSpell(m_spellInfo->Id) && !(m_spellInfo->Attributes & SPELL_ATTR0_CASTABLE_WHILE_MOUNTED))
    {
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        case SPELL_EFFECT_SKINNING:
        {
            ...
            if ((m_selfContainer && (*m_selfContainer) == this) && skillValue < sWorld->GetConfigMaxSkillValue() && (ReqValue < 0 ? 0 : ReqValue) > irand(skillValue - 25, skillValue + 37))
        ...
        {
            if (m_spellInfo->EffectImplicitTargetA[i] != TARGET_GAMEOBJECT && m_spellInfo->EffectImplicitTargetA[i] != TARGET_GAMEOBJECT_ITEM)
                break;
            ...
            if (m_spellInfo->EffectImplicitTargetA[i] == TARGET_GAMEOBJECT_ITEM && !m_targets.getGOTarget() && (!pTempItem || !pTempItem->GetProto()->LockID || !pTempItem->IsLocked()))
                return SPELL_FAILED_BAD_TARGETS;
        ...
        {
            if (m_spellInfo->Id == 57934 && m_targets.getUnitTarget() && m_targets.getUnitTarget()->GetTypeId() != TYPEID_PLAYER)
                return SPELL_FAILED_BAD_TARGETS;
            break;
        }
        case SPELL_EFFECT_LEAP_BACK:
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        case SPELL_AURA_AOE_CHARM:
        {
            ...
            if (m_spellInfo->EffectApplyAuraName[i] == SPELL_AURA_MOD_CHARM || m_spellInfo->EffectApplyAuraName[i] == SPELL_AURA_MOD_POSSESS)
            {
                if (m_caster->GetPetGUID())
                    return SPELL_FAILED_ALREADY_HAVE_SUMMON;
...
SpellCastResult Spell::CheckPetCast (Unit* target)
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (SpellTargetType[m_spellInfo->EffectImplicitTargetA[i]] == TARGET_TYPE_UNIT_TARGET || SpellTargetType[m_spellInfo->EffectImplicitTargetA[i]] == TARGET_TYPE_DEST_TARGET)
        {
            if (!target)
                return SPELL_FAILED_BAD_IMPLICIT_TARGETS;
            m_targets.setUnitTarget(target);
...
SpellCastResult Spell::CheckItems ()
{
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        case SPELL_EFFECT_ENCHANT_ITEM:
            if (m_spellInfo->EffectItemType[i] && m_targets.getItemTarget() && (m_targets.getItemTarget()->IsWeaponVellum() || m_targets.getItemTarget()->IsArmorVellum()))
            {
        ...
        case SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC:
        {
            ...
            for (uint8 e = 0; e < MAX_ITEM_PROTO_SPELLS; ++e)
            {
                ...
                if (proto->Spells[e].SpellId && (proto->Spells[e].SpellTrigger == ITEM_SPELLTRIGGER_ON_USE || proto->Spells[e].SpellTrigger == ITEM_SPELLTRIGGER_ON_NO_DELAY_USE))
                {
                    isItemUsable = true;
                    break;
                }
...
{
    return m_spellInfo->SpellVisual[0] || m_spellInfo->SpellVisual[1] || IsChanneledSpell(m_spellInfo) || m_spellInfo->speed > 0.0f || (!m_triggeredByAuraSpell && !m_IsTriggeredSpell);
}
...
SpellEvent::~SpellEvent ()
{
    ...
    {
        sLog->outError("~SpellEvent: %s %u tried to delete non-deletable spell %u. Was not deleted, causes memory leak.", (m_Spell->GetCaster()->GetTypeId() == TYPEID_PLAYER ? "Player" : "Creature"), m_Spell->GetCaster()->GetGUIDLow(), m_Spell->m_spellInfo->Id);
        ASSERT(false);
    }
}
